...
bool terminalTask::taskEntry()
{
    ...
    cp.addHandler(enableRCTestMode, "rctest", "Enable/Disable test mode for RC car. Ex: rctest 0/1");
    cp.addHandler(mockGeoController, "geo", "Send geo turning angle values. Ex: geo -90/90/0 for left/right/straight");
    cp.addHandler(mockBLEController, "ble","Send Ble user commands 1-start, 0-stop. Ex: ble 1");
    cp.addHandler(mockSensorController, "lidar", "Send track ids having obstacles for sectors-0,3,6,9. Ex: lidar 3 4 5 6");
