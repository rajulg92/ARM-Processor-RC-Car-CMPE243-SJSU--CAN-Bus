...
class Signal(object):
    ...
    def get_encode_code(self, raw_sig_name, var_name):
        ...
        # add condition in header if negative than use negative offset 
        code += "if ("+var_name+"< 0 "+") {// Check signed bit\n"
        code += raw_sig_code + "((int32_t)(((" + var_name + ") / " + str(self.scale) + ") - 0.5))"
        code += (" & 0x" + format(2 ** self.bit_size - 1, '02x') + ";\n")
        code += "}\nelse { \n"
        code += raw_sig_code +"((uint32_t)(((" + var_name + ") / " + str(self.scale) + ") + 0.5))"
        code += (" & 0x" + format(2 ** self.bit_size - 1, '02x') + ";\n")
        code += "\n}\n"
    ...
    def get_decode_code(self, raw_sig_name, prefix=''):
        ...
        if self.is_real_signed():
            ...
            s += "(((int32_t)((0xFFFFFFFF << " + str(self.bit_size - 1) + ") | " + raw_sig_name + ") * " + str(self.scale) + ") + (" + self.offset_str + "));\n"
            s += LINE_BEG + "} " + "else {\n"
    ...
    def gen_msg_hdr_instances(self):
        ...
        for mkey in self.messages:
            ...
            if not self.gen_all and not m.is_recipient_of_at_least_one_sig(
                ...
                code += "// "
            code += ("static constexpr dbc_msg_hdr_t " + (m.get_struct_name()[:-2] + "_HDR = ").ljust(32 + 7))
